...
impl Client {
    ...
    pub async fn create_response(&self, request: serde_json::Value) -> Result<serde_json::Value> {
        let res = self.inner.responses().create_raw(&request).await.map_err(|e| Error::from_reason(e.to_string()))?;
        Ok(res)
    }
    ...
    pub async fn ws_session(&self) -> Result<NodeWsSession> {
        let session = self.inner.ws_session().await
...
#[napi]
impl NodeWsSession {
    #[napi(ts_args_type="model: string, input: string", ts_return_type="Promise<Record<string, any>>")]
    pub async fn send(&self, model: String, input: String) -> Result<serde_json::Value> {
        ...
        if let Some(session) = lock.as_mut() {
            ...
            let response = session.send(req).await.map_err(|e| Error::from_reason(e.to_string()))?;
